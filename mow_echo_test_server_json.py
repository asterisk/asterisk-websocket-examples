...
async def send_file(ws_media, filename, chan_name, sent_buffer):
    ...
    await ws_media.send('{ "command": "START_MEDIA_BUFFERING" }')
    while True:
    ...
    f.close()
    await ws_media.send('{ "command": "MARK_MEDIA", "correlation_id": "666666666666" }')
#    await ws_media.send('{ "command": "STOP_MEDIA_BUFFERING", "correlation_id": "hfhfhrhrhdhs" }')
...
#    await ws_media.send('{ "command": "START_MEDIA_BUFFERING" }')
    while True:
        ...
        sent_buffer.write(buff)
    f.close()
    await ws_media.send('{ "command": "MARK_MEDIA", "correlation_id": "9999999999999" }')
    await ws_media.send('{ "command": "STOP_MEDIA_BUFFERING", "correlation_id": "aaaaaaaaa" }')
    logger.info(f"Stopping '{filename}' for {chan_name}")

async def check_data(ws_media, sent_buffer, recvd_buffer,
                     optimal_frame_size, timeout):
    global test_failed
    await ws_media.send('{ "command": "HANGUP" }')
    ...
    try:
        ...
        async for message in ws_media:
            if isinstance(message, str):
                ...
                if m['event'] == "MEDIA_START":
                    chan_name = m['channel']
                    optimal_frame_size = m['optimal_frame_size']
                    asyncio.create_task(send_file(ws_media,
                        "zombies.ulaw", chan_name, sent_buffer))
                elif m['event'] == "MEDIA_BUFFERING_COMPLETED":
